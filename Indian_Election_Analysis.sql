...
JOIN 
    ...
    ON cd.Constituency_ID = max_votes.Constituency_ID 
    AND cd.EVM_Votes = max_votes.Max_EVM_Votes
JOIN 
    constituencywise_results cr ON cd.Constituency_ID = cr.Constituency_ID
ORDER BY 
    cd.EVM_Votes DESC
...
-- 15. For the state of Bihar, what are the total number of seats, total number of candidates,
